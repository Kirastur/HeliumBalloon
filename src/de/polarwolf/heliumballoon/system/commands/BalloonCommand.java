...
public class BalloonCommand implements CommandExecutor {
	...
	protected void cmdDumpConfig(HeliumBalloonAPI api, CommandSender sender) {
		if (sender instanceof Player) {
			sender.sendMessage(api.getMessage(sender, Message.CONSOLE_ONLY_COMMAND, null));
			return;
		}
		for (String s : api.getConfigSectionNames()) {
			sender.sendMessage(api.dumpConfigSection(s));
		}
	}
	...
	protected void cmdInfo(HeliumBalloonAPI api, CommandSender sender) {
		if (api.isDisabled()) {
			...
			int count = api.getAllObservers().size();
