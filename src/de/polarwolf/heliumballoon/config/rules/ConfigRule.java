package de.polarwolf.heliumballoon.config.rules;
...
public class ConfigRule implements HeliumName {
	...
	protected HeliumSection heliumSection;
	protected List<HeliumParam> additionalRuleParams = new ArrayList<>();
	...
	public ConfigRule(String name, String fullName, HeliumSection heliumSection) throws BalloonException {
		this.name = name;
		this.fullName = fullName;
		this.heliumSection = heliumSection;
		importHeliumSection();
		validateConfig();
	}

	public ConfigRule(ConfigHelper configHelper, ConfigurationSection fileSection) throws BalloonException {
		this.name = fileSection.getName();
		this.fullName = fileSection.getCurrentPath();
		this.additionalRuleParams.addAll(configHelper.getAdditionalRuleParams());
	...
	public boolean getValueBoolean(HeliumParam param, boolean defaultValue) throws BalloonException {
		return heliumSection.getBoolean(param, defaultValue);
	}

	public int getValueInt(HeliumParam param, int defaultValue) throws BalloonException {
		return heliumSection.getInt(param, defaultValue);
	}

	public double getValueDouble(HeliumParam param, double defaultValue) throws BalloonException {
		return heliumSection.getDouble(param, defaultValue);
	}

	public String getValueString(HeliumParam param, String defaultValue) {
		return heliumSection.getString(param, defaultValue);
	}

	public List<HeliumParam> getValidRuleConfigParams() {
		List<HeliumParam> allParams = new ArrayList<>();
		allParams.addAll(Arrays.asList(ParamRule.values()));
		allParams.addAll(additionalRuleParams);
		return allParams;
	}

	protected void importHeliumSection() throws BalloonException {
	...
	protected void loadConfigFromFile(ConfigurationSection fileSection)
			throws BalloonException {
		heliumSection = new HeliumSection(fileSection, getValidRuleConfigParams());
		importHeliumSection();
		validateConfig();
	}
	...
	protected Map<String, String> buildMathParamMap() { // NOSONAR
		...
		if (heliumSection != null) {
			for (HeliumParam myParam : additionalRuleParams) {
				String s = heliumSection.getString(myParam);
				if ((s != null) && !s.isEmpty()) {
					newParamMap.put(myParam.getAttributeName(), s);
				}
			}
		}
		return newParamMap;
